...
gosu mitmproxy mitmdump --listen-port 3126 --ssl-insecure -s /home/mitmproxy/files/flows.py --set stream_large_bodies=10m --ignore-hosts '(mzstatic|apple|icloud|mobilesuica|crashlytic|google-analytics|merpay|paypay|rakuten-bank|fate|colopl|rakuten-sec|line|kyash|plexure)' --set block_global=false --set flow_detail=1 --showhost --anticomp --rawtcp --proxyauth ${MITM_USER}:${MITM_PASS}
